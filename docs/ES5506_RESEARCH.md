...
- MAME's `compute_tables()` initializes every voice with `CR=0x0003`,
  `LVOL=RVOL=0x8000`, and zero for the remaining voice fields. Its
  `es5506_device::device_reset()` then restores only `ACTV=0x1f` and
  `MODE=0x17`; voice registers, PAGE, and the IRQ vector are retained across
  that device reset. The shared RTL now performs the defaults on cold/download
  reset and preserves the voice banks across watchdog reset.
...
### Compressed sample decoding

MAME's `generate_ulaw()` selects the high byte of each fetched sample word
when `CR.CMPD` is set and decodes it with the OTTO equation generated by
`compute_tables()`. This is not a generic telephony u-law table. The shared
RTL helper `ssv_pkg::es5506_ulaw_decode()` keeps the raw value, exponent,
mantissa complement, and signed arithmetic in one place. The source-backed
checks in `verif/tb_ssv_es5506_ulaw.sv` cover the exponent-zero range, the
`0x7f/0x80` sign transition, and both rails; `verif/tb_ssv_es5506_voice.sv`
also verifies that the decoded signs survive the voice filter/mixer pipeline.
...
5. Hardware envelopes, loop modes, IRQ stacking, and compressed samples
   (implemented in the shared voice path and focused-tested against MAME).
