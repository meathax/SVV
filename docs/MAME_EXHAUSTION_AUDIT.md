...
The authoritative universal-profile manifest contains the nine supported sets
...
archive `ultraxg` is preserved for reference only and is not a profile entry or
proof target. The other 25 MAME entries remain an
...
| --- | ---: | --- | --- |
| Parent/profile proof scope | 9 | `dynagear`, `vasara`, `vasara2`, `cairblad`, `drifto94`, `stmblade`, `twineag2`, `ultrax`, `survartsu` | Current source-port and Verilator gameplay-proof targets, ordered by `PARENT_RUN_ORDER`; `survartsu` is the USA Survival Arts clone entry. |
| Retired local archives excluded from this pass | 1 | `ultraxg` | Private media remains intact, but the archive has no supported MRA or profile entry. |
...
These findings affect one or more of the nine supported targets or a universal
...
The next useful evidence is trace-driven coverage from all nine supported sets:
...
| P0 | Descriptor-visible geometry and matching blank status | Source-integrated; compile-checked | Four geometry profiles and exact edge points | Same-run non-empty screenshots at native geometry for all nine supported sets |
...
| P0 | Zero-valued unmapped reads | Source-integrated; compile-checked | Optional/no-read/write-only apertures plus mapped idle inputs | All-nine CPU progress/attract regression |
...
| P1 | Extra-input mode replacing boolean | Source-integrated; compile-checked | `none`, Dyna fixed-high, live Survival six-button | Dyna Gear and the new Survival Arts profile |
...
| P1 | V60 trace-driven semantic coverage | Decode exhausted; semantic coverage incomplete | Differential traces by opcode/address mode and IRQ boundary | Coverage from all nine supported sets |
