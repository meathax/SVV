...
`ifdef SSV_VISUAL
`define SSV_SAMPLE_ROM_MODEL
`elsif SSV_HEADLESS_SAMPLE_ROM
`define SSV_SAMPLE_ROM_MODEL
`endif
...
ssv_core dut (
    ...
    .in_system(in_system), .in_extra(in_extra), .in_mahjong_rows(24'hffffff),
...
`ifdef SSV_SAMPLE_ROM_MODEL
    ...
    if (sample_count != sample_bytes)
        $fatal(1, "short sample ROM read %0d/%0d",
               sample_count, sample_bytes);
    if (sample_word_be(8'hcd, 8'hab) !== 16'hcdab)
        $fatal(1, "sample endian proof failed");
    $display("SSV_SAMPLE_ENDIAN_PROOF bytes=cd,ab word=%04x",
             sample_word_be(8'hcd, 8'hab));
    $display("SSV_SAMPLE_ROM_LOADED path=%s bytes=%0d first_word=%04x last_word=%04x",
