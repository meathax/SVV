...
function automatic logic enabled_now();
    enabled_now = (cycle >= start_cycle) &&
                  ...
                  (!frame_window_enabled ||
                   ((post_epoch_frame >= start_frame) &&
                    (post_epoch_frame <= stop_frame)));
endfunction
...
initial begin
    ...
    if ($value$plusargs("TRACE_START_FRAME=%d", start_frame))
        frame_window_enabled = 1'b1;
    if ($value$plusargs("TRACE_STOP_FRAME=%d", stop_frame))
        frame_window_enabled = 1'b1;
    if (frame_window_enabled && (stop_frame < start_frame))
