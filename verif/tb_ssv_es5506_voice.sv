...
ssv_es5506_voice dut (
    ...
    .eng_wr_accum, .eng_accum_w, .eng_wr_cr, .eng_cr_set, .eng_cr_clr,
...
always_ff @(posedge clk) begin
    if (!rst) begin
        ...
        if (eng_wr_cr) eng_cr <= (eng_cr | eng_cr_set) & ~eng_cr_clr;
...
initial begin
    ...
    // Drift Out '94's first audible voice programming is identical in the
    // paired MAME/RTL host-write prefix. Exercise those exact values and the
    // first two addressed ROM words so a full-core integration fault cannot
    // be mistaken for attenuation in the voice arithmetic itself.
    defaults();
    cfg = cfg_drifto94();
    samp[8'h42] = 16'hc400;
    samp[8'h43] = 16'hd000;
    ...
    eng_k1 = 16'hfff0;
    eng_k2 = 16'hfff0;
    eng_start = 32'heff2_1000;
    eng_end = 32'hf1d6_8000;
    eng_accum = 32'heff2_1000;
    expected_sdr_addr = samp_base + (25'd1 << 21) + 25'h1dfe42;
    check_sdr_addr = 1'b1;
    reset_dut();
    wait_sample();
    check_sdr_addr = 1'b0;
    if (sdr_fetches < 2 || audio_l > -16'sd512 || audio_r > -16'sd512)
        $fatal(1, "Drift Out prefix voice was unexpectedly quiet: fetches=%0d L=%0d R=%0d",
               sdr_fetches, audio_l, audio_r);
