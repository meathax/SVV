...
initial begin
    ...
    // On real hardware the free-running voice engine would have snapshotted
    // voice 16 (S_START) at least once since the ACCUM host write above,
    // clearing the register file's "host wrote a fresher value since this
    // voice's snapshot" tracking for that field. This standalone regs-only
    // testbench drives the engine side by hand, so it must pulse eng_snap
    // itself -- otherwise that tracking would still show ACCUM host-fresh
    // from line 229 and wrongly suppress the deliberate engine ACCUM
    // writeback this test is about to exercise.
    @(negedge clk);
    eng_voice = 5'd16;
    eng_snap  = 1'b1;
    @(negedge clk);
