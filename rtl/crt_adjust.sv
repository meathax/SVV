...
// LOCAL ADDITION (the only edit to the vendored upstream file): every other
// design element in this project declares a time scale, and a design that
// mixes the two is a hard error to slang and a warning to Verilator. Quartus
// ignores it. Drop this line if the file is ever re-vendored into a project
