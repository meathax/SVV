...
def normalize_trace(
    ...
    *,
    ...
    require_receipt = contract.get("rules", {}).get("require_complete_receipt") is True
    ...
    with tempfile.TemporaryDirectory(prefix=".mister-normalize-", dir=str(output_path.parent)) as td:
                ...
                if record is not None:
                    if record == "barrier":
                        if receipt is not None:
                            raise TraceError(f"{raw_path}:{line_number}: barrier appears after final receipt.")
                        barrier_count += 1
                        continue
                    if record == "receipt":
                        if receipt is not None:
                            raise TraceError(f"{raw_path}:{line_number}: duplicate final receipt.")
                        receipt = raw
                        continue
                    raise TraceError(f"{raw_path}:{line_number}: unknown control record {record!r}.")
                if receipt is not None:
                    raise TraceError(f"{raw_path}:{line_number}: event appears after final receipt.")
        ...
        if require_receipt:
            if receipt is None:
                raise TraceError(f"{raw_path}: trace is missing its final completeness receipt.")
            if receipt.get("complete") is not True:
                raise TraceError(f"{raw_path}: trace receipt does not declare complete=true.")
            if receipt.get("dropped") != 0:
                raise TraceError(f"{raw_path}: trace receipt reports dropped events.")
            if barrier_count < 1:
                raise TraceError(f"{raw_path}: trace has no completed semantic barrier.")
            receipt_counts = receipt.get("counts")
            if not isinstance(receipt_counts, dict):
                raise TraceError(f"{raw_path}: trace receipt counts must be an object.")
            for domain, count in counts.items():
                if receipt_counts.get(domain) != count:
                    raise TraceError(
                        f"{raw_path}: receipt count for {domain!r} is {receipt_counts.get(domain)!r}; "
                        f"parsed {count}."
                    )
            unexpected = sorted(set(receipt_counts) - set(counts))
            if unexpected:
                raise TraceError(f"{raw_path}: receipt names unparsed domains: {', '.join(unexpected)}")
    ...
    return {
        ...
        "event_count": sum(counts.values()),
        "domains": dict(sorted(counts.items())),
        "receipt_verified": require_receipt,
