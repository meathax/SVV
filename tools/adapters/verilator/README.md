...
Call `barrier()` at every semantic stop used for comparison and `close()` only
after the stop barrier is complete. The final receipt binds per-domain counts
and declares zero dropped events. A trace without that receipt is truncated or
unattested and cannot establish a missing-side divergence.
