...
Every eligible capture must end with a `barrier` record and a `receipt` record.
The receipt reports the final per-domain event counts, `complete=true`, and
`dropped=0`. A missing receipt, a non-contiguous sequence, or a receipt count
that disagrees with the parsed events invalidates the capture; it must never be
interpreted as an event that is absent from the emulated machine.
