...
def emit_region(region, indent='    ', stream_end=None):
    ...
    while i < len(loads):
        ...
        if ld['kind'] == 'ROM_LOAD16_WORD_SWAP':
            # mra-tools map digits are source-byte offsets read right-to-left:
            # "12" emits src[1],src[0] per word (byte swap), "21" emits
            # src[0],src[1] (identity). MAME's *_WORD_SWAP swaps bytes into the
            # big-endian region, so the swapping map is required here; "21"
            # shipped byte-swapped 16-bit PCM and played as loud static.
            out.append('%s<interleave output="16">' % indent)
            out.append('%s  <part name="%s" crc="%s" map="12"/>'
                       % (indent, xml_escape(ld['name']), ld['crc']))
            out.append('%s</interleave>' % indent)
        else:
...
def main():
    ...
    for setname in order:
        ...
        if setname in DDR_FAST_LOAD_SETS:
            ...
            # streaming it over ioctl; rtl/mem/ssv_ddr_rom_loader.sv reads
            # it back. Must match that module's DDR_BASE_BYTES parameter.
            out.append('  <rom index="0" zip="%s" address="%s">' %
                       (zips, DDR_FAST_LOAD_ADDR))
        else:
